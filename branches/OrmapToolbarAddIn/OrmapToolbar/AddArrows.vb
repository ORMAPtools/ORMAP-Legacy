...
#Region "Event Handlers"
    ...
    Private Sub PartnerAddArrowsForm_Close(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs)
        If arrowType = "" Then resetTool(Keys.Q) 'exit tool
    End Sub
...
#Region "Methods"
    ...
    Public Sub resetTool(theKeyCode As Keys)

        ' End the dimension arrow tool if the "q" key is pressed
        If theKeyCode = System.Windows.Forms.Keys.Q Then
            If arrowType.Equals("Dimension", _ignoreCase) Then
                ...
                my.arcmap.application.RefreshWindow()
            ...
            End If
        End If
        If theKeyCode = System.Windows.Forms.Keys.D Then
            If arrowType.Equals("Dimension", _ignoreCase) Then
    ...
    Private Sub getMousePoint(ByRef X As Integer, ByRef Y As Integer)
        ...
        Try
            ...
            Dim theSnapenv As ISnapEnvironment = DirectCast(My.ArcMap.Editor, ISnapEnvironment)
            ...
            For i As Integer = 0 To theSnapenv.SnapAgentCount - 1
                ...
                If TypeOf theSnapAgent Is IFeatureSnapAgent Then
                    ...
                    theEdLyrs = DirectCast(My.ArcMap.Editor, IEditLayers)
    ...
    Private Function getCurrentMapScale(ByRef theMIFeatureClass As IFeatureClass) As String
        ...
        Try
            ...
            currentMapScale = GetValue((theDimensionFeatureTemp.Shape), theMIFeatureClass, OrmapExtension.MapIndexSettings.MapScaleField, OrmapExtension.MapIndexSettings.MapNumberField)
...
#Region "Methods"
    ...
    '                my.arcmap.application = DirectCast(hook, IApplication)
    '                setPartnerAddArrowsForm(New AddArrowsForm())
    '                setPartnerDimensionArrowsForm(New DimensionArrowsForm())
            ...
            Select Case arrowType
                ...
                Case "ARROW" 'If drawing annotation arrows
                    If finishedDrawing = False Then
                        If arg.Button = 1 AndAlso arg.Shift = 1 Then 'Right mouse click
                            'Save the first point
                            If _arrowPointsCollection.Count = 0 Then
                                If _theArrowPt1 Is Nothing Then
                                    _theArrowPt1 = _theActiveView.ScreenDisplay.DisplayTransformation.ToMapPoint(arg.X, arg.Y)
                            ...
                            'Left mouse click
                        ElseIf arg.Button = 1 AndAlso arg.Shift = 0 Then
                            'Add vertex
                            If _theArrowPt1 Is Nothing Then
                                _theArrowPt1 = _theActiveView.ScreenDisplay.DisplayTransformation.ToMapPoint(arg.X, arg.Y)
                                _arrowPointsCollection.Add(_theArrowPt1)
                    ...
                    If _arrowPointsCollection.Count > 1 AndAlso finishedDrawing = True Then
                        ...
                        Dim mapNumberField As Integer = LocateFields(theArrowFeatureClass, OrmapExtension.MapIndexSettings.MapNumberField)
                        theField = LocateFields(theArrowFeatureClass, OrmapExtension.CartographicLinesSettings.LineTypeField)
                        If theField = NotFoundIndex Then Exit Sub
                        ' Populate field values in the feature
                        Dim curMapNum As String = GetValue(theFeature.Shape, theMIFeatureClass, OrmapExtension.MapIndexSettings.MapNumberField, OrmapExtension.MapIndexSettings.MapNumberField)
                        theFeature.Value(mapNumberField) = curMapNum
                        theFeature.Value(theField) = arrowLineStyle
                        theFeature.Store()

                        ' Set the AutoMethod Field
                        theField = LocateFields(theArrowFeatureClass, OrmapExtension.AllTablesSettings.AutoMethodField)
                        If theField = NotFoundIndex Then Exit Sub
                        theFeature.Value(theField) = "UNK"

                        ' Set the AutoWho Field
                        theField = LocateFields(theArrowFeatureClass, OrmapExtension.AllTablesSettings.AutoWhoField)
                        If theField = NotFoundIndex Then Exit Sub
                        theFeature.Value(theField) = OrmapTaxlotEditing.Utilities.UserName

                        ' Set the AutoDate Field
                        theField = LocateFields(theArrowFeatureClass, OrmapExtension.AllTablesSettings.AutoDateField)
                        If theField = NotFoundIndex Then Exit Sub
                        theFeature.Value(theField) = Format(Today, "MM/dd/yyyy")

                        ' Set the MapScale Field
                        currentMapScale = GetValue(theFeature.Shape, theMIFeatureClass, OrmapExtension.MapIndexSettings.MapScaleField, OrmapExtension.MapIndexSettings.MapNumberField)
                        theField = LocateFields(theArrowFeatureClass, OrmapExtension.MapIndexSettings.MapScaleField)
                        If theField = NotFoundIndex Then Exit Sub
                ...
                Case "DIMENSION"
                    ...
                    ElseIf _theArrowPt3 Is Nothing Then
                        _theArrowPt3 = _theActiveView.ScreenDisplay.DisplayTransformation.ToMapPoint(arg.X, arg.Y)
                        drawArrows()
                        ...
                        For indexNumber = 1 To 2
                            ...
                            If _addManually = False Then
                                ...
                                If indexNumber = 1 Then
                                    If arg.Shift = 0 Then 'shift not pressed
                                        ...
                                        ElseIf Trim$(UCase$(dimensionArrowSide)) = "LEFT" Then
                                            theDimensionLine.QueryNormal(esriSegmentExtension.esriExtendAtFrom, iChange, False, -((iChange / RatioLine) / 2), theLine)
                                        End If
                                    ElseIf arg.Shift = 1 Then 'shift pressed
                                        If Trim$(UCase$(dimensionArrowSide)) = "RIGHT" Then
                            ...
                            theField = LocateFields(theDimensionArrowFC, OrmapExtension.CartographicLinesSettings.LineTypeField)
                            If theField = NotFoundIndex Then Exit Sub
                            theDimensionFeature.Value(theField) = 134 'Bearing Distance Arrow

                            ' Get the current MapNumber
                            Dim currentMapNums As String
                            currentMapNums = GetValue(theDimensionFeature.Shape, theMIFeatureClass, OrmapExtension.MapIndexSettings.MapNumberField, OrmapExtension.MapIndexSettings.MapNumberField)
                            theField = LocateFields(theDimensionArrowFC, OrmapExtension.MapIndexSettings.MapNumberField)
                            theDimensionFeature.Value(theField) = currentMapNums

                            ' Set the AutoMethod Field
                            theField = LocateFields(theDimensionArrowFC, OrmapExtension.AllTablesSettings.AutoMethodField)
                            If theField = NotFoundIndex Then Exit Sub
                            theDimensionFeature.Value(theField) = "UNK"

                            ' Set the AutoWho Field
                            theField = LocateFields(theDimensionArrowFC, OrmapExtension.AllTablesSettings.AutoWhoField)
                            If theField = NotFoundIndex Then Exit Sub
                            theDimensionFeature.Value(theField) = OrmapTaxlotEditing.Utilities.UserName

                            ' Set the AutoDate Field
                            theField = LocateFields(theDimensionArrowFC, OrmapExtension.AllTablesSettings.AutoDateField)
                            If theField = NotFoundIndex Then Exit Sub
                            theDimensionFeature.Value(theField) = Format(Today, "MM/dd/yyyy")

                            ' Set the MapScale Field
                            theField = LocateFields(theDimensionArrowFC, OrmapExtension.MapIndexSettings.MapScaleField)
                            If theField = NotFoundIndex Then Exit Sub
        ...
        Try
            ...
            If arrowType.Equals("dimension", _ignoreCase) Then
                ...
                If _mouseHasMoved Then
                    ...
                    End If
                    ' erase the old agent
                    My.ArcMap.Editor.InvertAgent(_thePt, 0)
                    ' get the new point
                    getMousePoint(arg.X, arg.Y)
                Else
                    _thePt = _theActiveView.ScreenDisplay.DisplayTransformation.ToMapPoint(arg.X, arg.Y)
                End If
                _mouseHasMoved = True

                If Not _theSnapAgent Is Nothing Then
                    Dim thesnapenv As ISnapEnvironment
                    thesnapenv = DirectCast(My.ArcMap.Editor, ISnapEnvironment)
                ...
                My.ArcMap.Editor.InvertAgent(_thePt, 0) 'draw the agent
