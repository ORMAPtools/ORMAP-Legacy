...
<ComClass(RotateAnnotation.ClassId, RotateAnnotation.InterfaceId, RotateAnnotation.EventsId), _
 ProgId("OrmapTaxlotEditing.RotateAnnotation")> _
Public NotInheritable Class RotateAnnotation
    ...
    Private Sub performRotation()
        ...
        Do While Not (thisAnnoLayer Is Nothing)
            ...
            If Not theSelectedAnnoCursor Is Nothing Then
                ...
                'Took a long time to figure out this bit of ESRI nonsense... probably related to the fact
                'that annotation feature classes are not true feature classes, but feature dataset graphics objects. 
                Dim theGraphicsContainer As IGraphicsContainer = DirectCast(theMxDoc.ActivatedView, IGraphicsContainer)

                'Now loop through the selection set and rotate the elements
                'TODO:  Refactor... this is rife with issues and errors (what if user picks odd number of annotation features, etc)
                Dim i As Integer
                For i = 1 To theAnnoSelection.SelectionSet.Count
                    'Get the selected features (needed for the transform2d since it accesses the underlying geometry of the annotation)
                    Dim theSelectedFeature As IFeature = theSelectedAnnoCursor.NextFeature

                    'Get the selected annotation features annotation elements (needed to do transform and update the graphics container)
                    Dim theSelectedAnnoFeature As IAnnotationFeature = DirectCast(theSelectedFeature, IAnnotationFeature)

                    'Calculate a mean X, Y pair for the 1st and 2nd element's envelopes (these will be the move vector's end points)
                    Dim theRotationPoint As IPoint = CalculateCentroid(theSelectedAnnoFeature.Annotation.Geometry.Envelope)

                    RotateElement(theRotationPoint, theGraphicsContainer, theSelectedAnnoFeature.Annotation)
                Next
